...
# In slack someone suggested FrameView{T <: AbstractVector} <: AbstractVector{T}
struct FrameView{T<:AbstractVector} # <: AbstractVector{Vector}
    ...
    function FrameView(original::T,framesize::Int,hopsize::Int) where {T<:AbstractVector}
        nframes = div(length(original) - framesize, hopsize) + 1
        return new{T}(original,framesize,hopsize,nframes)
    end
...
# Base.iterate(x::FrameView) = (x[i], i+1)
...
Base.iterate(x::FrameView) = iterate(x.original)
Base.iterate(x::FrameView,i) = iterate(x.original,i)
Base.size(x::FrameView) = (x.nframes,)
